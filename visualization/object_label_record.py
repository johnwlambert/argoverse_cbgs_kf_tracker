...
def add_text_cv2(img, text, coords_to_plot_at=None, font_color=(0,0,0), font_scale = 1, thickness=2):
    """
    font_color = (0,0,0)
    ...
    line_type = 2
    img = cv2.putText(img=img, text=text, 
...
class ObjectLabelRecord:
    ...
    def render_clip_frustum_cv2(
        ...
        text_coords, _, _, _ = proj_cam_to_uv(center_bottom.reshape(1, 3), camera_config)

        text_coords = tuple(list(text_coords.squeeze().astype(np.int64)))

        img = add_text_cv2(img, plot_text, coords_to_plot_at=text_coords, font_color=(0,0,255), font_scale=2)
